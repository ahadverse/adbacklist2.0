...
const MultipleCitiesSelection = ({ setCitiesModal, setShowCities }) => {
    ...
    <div className='w-full'>
      ...
      <div className='bg-white flex px-1 justify-between items-center'>
        ...
        <div>
          {state.length ? (
            ...
            >
              Save {isLoading ? <ImSpinner10 className='animate-spin' /> : ""}
            </button>
          ) : (
            ""
          )}
      ...
      {cities?.[0]?.children?.map((c) => (
        <div className='collapse collapse-arrow' key={c._id}>
          ...
          <div className='collapse-title border bg-white rounded-lg text-pink-800   peer-checked:bg-white peer-checked:text-pink-800'>
            {c?.name}
          </div>
